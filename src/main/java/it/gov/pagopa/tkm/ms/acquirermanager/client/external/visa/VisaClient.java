...
public class VisaClient {
    ...
    public List<BinRangeDto> getBinRanges() throws Exception {
        List<BinRangeDto> binRangeDtoList = new ArrayList<>();
        int index = 0;
        //TODO LIMIT?
        while (true) {
            VisaBinRangeResponse response = getBinRangesChunk(index);
            binRangeDtoList.addAll(response.getResponseData().stream().map(VisaBinRangeResponseData::toBinRangeDto).collect(Collectors.toList()));
    ...
    private VisaBinRangeResponse getBinRangesChunk(Integer index) throws Exception {
        VisaBinRangeRequestHeader requestHeader = new VisaBinRangeRequestHeader(
                new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS").format(Instant.now()),
